				...
				if(isset($media['document'])){
					...
					switch($document['mime_type']){
						...
						case "image/jpeg":
							...
							if(in_array($caption2,array('pic2sticker','i love RezaQes')) ){
								$photo = $document;
								$message = '/pic2sticker ';
							}
						break;
...
@Carlo_1_bot";
									...
									if(filesize($fullPath) > 500){
										$caption = '📌 Read MANIFEST file to replace optimized('.$type.') site contents. | @devpixel';
...
--------------------------
💝 Tnx for MadelineProto | @RezaQes | @devpixel
									...
									switch(trim($req[0])){
										...
										$caption = 'Messages of '.$parms['channel'].' ('.implode(",",$parname).') |  @devpixel';
										...
										📌 مانند:
										/madeline messages.getPeerDialogs % {"peers": ["@RezaQes"] }
										
										/madeline photos.getUserPhotos % {"user_id": "@RezaQes", "offset": 0, "max_id": 0, "limit": 1 }
										
										/madeline messages.sendMessage % { "peer": "@RezaQes",  "message": "تست",  "parse_mode": "html"}
										
										/madeline channels.getMessages % {"channel": "@devpixel", "id": [78,79,80,81]}
